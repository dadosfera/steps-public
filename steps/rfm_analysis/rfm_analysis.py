...
def rfm_analysis(secret_id: str, table_identifier: str, customer_id_col: str, date_col: str, monetary_col: str) -> List[str]:
    session = get_snowpark_session(secret_id)
    session.sql(f"""CREATE OR REPLACE TABLE {table_identifier}_rfm AS 
                        WITH model AS (
                            SELECT
...
def orchest_handler():
    ...
    queries = rfm_analysis(secret_id=secret_id, table_identifier=table_identifier, customer_id_col=customer_id_col, date_col=date_col, monetary_col=monetary_col)
